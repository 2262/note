...
常用算法
概念：算法主要是由头文件<algorithm><functional><numeric>纟且成·G1gorithm>是所有STL头文亻牛中最大的一个，范围涉及到比较、交、查然历操作、复制、修改等等·<numeric>体积很小，只包括几个在序列上面进行简单数学运算的模板函数·<functional>疋义了一些模板类，用以明函数对象。
### 迭代器：容器和算法之间粘合剂
