...
- 这种继承被称为菱形继承，或者钻石继承
**菱形继承的典型案例：
![](../../resources/Pasted%20image%2020250405173708.png)
菱形继承的问题：
1. 羊继承了动物的数据，驼同样继承了动物的数据，当草泥马使用数据时就会产生二义性
2. 草泥马继承自动物的数据继承了两份，其实我们很清楚，这份数据我们只需要一份就可
```cpp
#include <iostream>
using namespace std;
...
public:
    void run() {
        cout << "SheepCamel::run()" << endl;
    }

    void eat() {
        cout << "SheepCamel::eat()" << endl;
    }
...
void test01() {
    ...
    sc.Camel::m_Name = "大羊驼";
    sc.Camel::m_Age = 20;
    //当菱形继承时，如果子类中存在同名的成员变量，则需要使用作用域限定符进行区分
    cout << "sc.m_Name = " << sc.Sheep::m_Name << "sc.m_Age = " << sc.Sheep::m_Age << endl;
    sc.run();
    sc.eat();
    //菱形继承导致数据有两份，造成资源浪费
}
